...
import { ShellIcon, WarningIcon } from '../common/Icons';
...
const ReverseShellGenerator: React.FC = () => {
  ...
  return (
    <ToolWrapper
      ...
      icon={<ShellIcon />}
      description="Generate reverse shell payloads for penetration testing"
    >
      <div className={styles.section}>
        <div className={styles.inputGroup}>
          <label className={styles.label}>Listener IP Address</label>
          <input
            type="text"
        ...
        <div className={styles.inputGroup}>
          <label className={styles.label}>Listener Port</label>
          <input
            type="text"
            className={styles.input}
            value={port}
      ...
      <div className={styles.resultBox}>
        <div className={styles.resultHeader}>
          <span className={styles.resultTitle}>Generated Payload</span>
          <button onClick={copyToClipboard} className={styles.copyBtn}>
            Copy
          </button>
        </div>
        <pre className={styles.codeBlock}>{shells[shellType as keyof typeof shells]}</pre>
      </div>
      ...
      <div className={styles.infoBox}>
        ...
        <ol>
          <li>
            Set up listener: <code className={styles.code}>nc -lvnp {port}</code>
          </li>
          <li>Execute payload on target system</li>
          <li>Receive connection on your listener</li>
        </ol>
        <p className={styles.warningBox} style={{ marginTop: '12px', marginBottom: 0 }}>
          <WarningIcon /> Only use on systems you own or have permission to test
        </p>
      </div>
    </ToolWrapper>
  );
