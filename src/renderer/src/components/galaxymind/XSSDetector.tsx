...
export default function XSSDetector() {
  ...
  const test = async () => {
    ...
    for (let i = 0; i < payloads.length; i++) {
      ...
      try {
        ...
        if (response.success && response.data) {
          ...
          if (body.includes(payload.toLowerCase()) || body.includes(payload.replace(/"/g, "'"))) {
            vulnerabilities.push(`Potential XSS with: ${payload.slice(0, 40)}...`);
          }
        }
      } catch {}
    ...
    setResults(
      vulnerabilities.length > 0 ? vulnerabilities : ['No obvious XSS vulnerabilities detected']
    );
    setLoading(false);
  };
      ...
      <div className={styles.section}>
        ...
        <div className={styles.buttonGroup}>
          <button className={styles.primaryBtn} onClick={test} disabled={loading}>
            {loading ? `Testing... ${progress}%` : 'Test XSS'}
          </button>
          <button
